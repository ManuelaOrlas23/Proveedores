...
@RestController
@RequestMapping("demo/v1/proveedor")
public class ProveedorControlador {
    ...
    @PostMapping
    public ResponseEntity<?> guardarProovedor(@RequestBody Proveedor datosProveedor){
        try {
    ...
    public ResponseEntity<?> buscarProveedorPorId(@PathVariable Integer id){
        try {
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(proveedorServicio.buscarProovedorPorId(id));
        }catch (Exception error){
            Map<String,Object> errorDetails = new LinkedHashMap<>();
            errorDetails.put("timestamp",LocalDateTime.now());
            errorDetails.put("message",error.getMessage());
